...
with sync_playwright() as p:
      ...
      time.sleep(1)
      produto = pyautogui.locateOnScreen('produto.png', confidence=0.9)
      pyautogui.moveTo(produto)
      posx= pyautogui.moveTo(x=produto.left)
      posy= pyautogui.moveTo(y=produto.top)
      print("posição x: "+posx+" posição y: "+posy+"")
      mxm.mouse.click(x = int(posx), y = int(posy))
      print(produto)
      time.sleep(10)
      time.sleep(0.5)
      with context.expect_event("tab") as event_info:
          page.locator("#tabMain__1022").click()
      product = event_info.value
      print(product)
      product.locator("#hpfCodigo").fill("0881")
    #   product.locator("#chkControleLiberadoParaMovimentacao").check()
      print(produto)
      time.sleep(5)
      browser.close()
